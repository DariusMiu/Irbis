...
{   //start at line 11 to make it easier to count, just subtract 10 from final line
    ...
    public static void Save(PlayerSettings settings, string filename)
    {
        ...
        if (debug0) { Console.WriteLine("file directory: " + filename); }
        ...
        //sb.AppendLine("XcolliderOffset=" + settings.XcolliderOffset);
        //sb.AppendLine("YcolliderOffset=" + settings.YcolliderOffset);
        //sb.AppendLine("colliderWidth=" + settings.colliderWidth);
        //sb.AppendLine("colliderHeight=" + settings.colliderHeight);
        ...
        if (debug0) { Console.WriteLine(";lines beginning with \";\" and empty lines are ignored"); }
         writer.WriteLine(";delete this file and relaunch to return everything to defaults");
        if (debug0) { Console.WriteLine(";delete this file and relaunch to return everything to defaults"); }
         writer.WriteLine("");
         writer.WriteLine("");


         writer.WriteLine(";SETTINGS");
        if (debug0) { Console.WriteLine(";SETTINGS"); }
         writer.WriteLine(";KEY BINDS");
        if (debug0) { Console.WriteLine(";KEY BINDS"); }
        ...
        if (debug0) { Console.WriteLine(";CAMERA SETTINGS"); }
        ...
        if (debug0) { Console.WriteLine(";VIDEO SETTINGS"); }
        ...
        if (debug0) { Console.WriteLine(";AUDIO SETTINGS"); }
        ...
        if (debug0) { Console.WriteLine(";MISC SETTINGS"); }
        ...
        if (debug0) { Console.WriteLine(";CHEATS"); }
        ...
        if (debug0) { Console.WriteLine(";ANIMATION SETTINGS"); }
        ...
        if (debug0) { Console.WriteLine(";FONT SETTINGS"); }
        ...
        if (debug0) { Console.WriteLine(";DEBUG MODE"); }
         writer.WriteLine("debug=" + settings.debug);
         writer.WriteLine("");
         writer.WriteLine(";//END");


        if (debug0) { Console.WriteLine("save directory: " + ((FileStream)(writer.BaseStream)).Name); }
    ...
    public static void Save(Irbis.Irbis game, string filename)
    {
        ...
        writer.WriteLine(";a list of all available keys can be found here:");
        writer.WriteLine("; https://msdn.microsoft.com/en-us/library/microsoft.xna.framework.input.keys.aspx");
        writer.WriteLine("attackKey=" + game.attackKey);
        writer.WriteLine("altAttackKey=" + game.altAttackKey);
        writer.WriteLine("");
        writer.WriteLine("shockwaveKey=" + game.shockwaveKey);
        writer.WriteLine("altShockwaveKey=" + game.altShockwaveKey);
        writer.WriteLine("");
        writer.WriteLine("shieldKey=" + game.shieldKey);
        writer.WriteLine("altShieldKey=" + game.altShieldKey);
        writer.WriteLine("");
        writer.WriteLine("jumpKey=" + game.jumpKey);
        writer.WriteLine("altJumpKey=" + game.altJumpKey);
        writer.WriteLine("");
        writer.WriteLine("upKey=" + game.upKey);
        writer.WriteLine("altUpKey=" + game.altUpKey);
        writer.WriteLine("");
        writer.WriteLine("downKey=" + game.downKey);
        writer.WriteLine("altDownKey=" + game.altDownKey);
        writer.WriteLine("");
        writer.WriteLine("leftKey=" + game.leftKey);
        writer.WriteLine("altLeftKey=" + game.altLeftKey);
        writer.WriteLine("");
        writer.WriteLine("rightKey=" + game.rightKey);
        writer.WriteLine("altRightKey=" + game.altRightKey);
        writer.WriteLine("");
        writer.WriteLine("rollKey=" + game.rollKey);
        writer.WriteLine("altRollKey=" + game.altRollKey);
        writer.WriteLine("");
        writer.WriteLine("potionKey=" + game.potionKey);
        writer.WriteLine("altPotionKey=" + game.altPotionKey);
        writer.WriteLine("");
        writer.WriteLine("");
        ...
        writer.WriteLine(";where location refers to the center of the rectangle");
        writer.WriteLine("boundingBox={X:" + game.boundingBox.Center.X + " Y:" + game.boundingBox.Center.Y + " Width:" + game.boundingBox.Width + " Height:" + game.boundingBox.Height + "}");
        writer.WriteLine("");
        writer.WriteLine(";Do you want the camera to smoothly trail the player?");
        writer.WriteLine("cameraLerp=" + game.cameraLerp);
        writer.WriteLine("");
        writer.WriteLine(";How fast should the camera lerp?");
        writer.WriteLine("cameraLerpSpeed=" + game.cameraLerpSpeed);
        writer.WriteLine("");
        writer.WriteLine(";Turn off camera \"swing\" (the motion the camera makes when you attack and miss)");
        writer.WriteLine("cameraSwingSetting=" + game.cameraSwingSetting);
        writer.WriteLine("");
        writer.WriteLine(";How far should the camera swing? (pixels)");
        ...
        writer.WriteLine("");
        writer.WriteLine(";Turn off camera shake");
        writer.WriteLine("cameraShakeSetting=" + game.cameraShakeSetting);
        writer.WriteLine("");
        writer.WriteLine("");
        ...
        if (debug0) { Console.WriteLine(";VIDEO SETTINGS"); }
        writer.WriteLine(";toggles windowed/fullscreen mode");
        writer.WriteLine("fullscreen=" + game.graphics.IsFullScreen);
        writer.WriteLine("");
        writer.WriteLine(";the scale of the window");
        writer.WriteLine("screenScale=" + game.screenScale);
        writer.WriteLine("");
        writer.WriteLine(";how much of the world will be drawn");
        writer.WriteLine("resolution=" + game.tempResolution);
        writer.WriteLine(";the actual size of the window is a combination of resolution and scale");
        writer.WriteLine(";for example, a 960x540 resolution at 2x scale gives you a 1920x1080 window");
        writer.WriteLine(";with each in-game pixel using 2x2 pixels on your screen");
        writer.WriteLine("");
        ...
        writer.WriteLine(";AUDIO SETTINGS");
        if (debug0) { Console.WriteLine(";AUDIO SETTINGS"); }
        writer.WriteLine("masterAudioLevel=" + game.masterAudioLevel);
        writer.WriteLine("musicLevel=" + game.musicLevel);
        writer.WriteLine("soundEffectsLevel=" + game.soundEffectsLevel);
        writer.WriteLine("");
        writer.WriteLine("");
        ...
        writer.WriteLine("");
        writer.WriteLine(";How long before the player is considered idle (seconds)");
        writer.WriteLine("idleTimeMax=" + game.geralt.idleTimeMax);
        writer.WriteLine("");
        writer.WriteLine(";This is the time it takes to \"charge\" the super shockwave (seconds)");
        writer.WriteLine(";Basically, how long you have to hold the shockwaveKey to use");
        writer.WriteLine("superShockwaveHoldtime=" + game.geralt.superShockwaveHoldtime);
        writer.WriteLine("");
        writer.WriteLine(";The amount of time the player can hold down the left or right movement key");
        writer.WriteLine(";during a wall jump before they drift away from the wall (seconds)");
        writer.WriteLine("walljumpHoldtime=" + game.geralt.walljumpHoldtime);
        writer.WriteLine("");
        writer.WriteLine(";Minimum distance(squared) for an enemy health bar to appear(bosses override this)");
        ...
        if (debug0) { Console.WriteLine(";CHEATS"); }
        writer.WriteLine(";self-explanatory");
        writer.WriteLine("attack1Damage=" + game.geralt.attack1Damage);
        writer.WriteLine("attack2Damage=" + game.geralt.attack2Damage);
        writer.WriteLine("");

        writer.WriteLine(";run&jump speed");
        writer.WriteLine("speed=" + game.geralt.speed);
        writer.WriteLine(";The maximum speed you can travel in any given direction");
        writer.WriteLine("terminalVelocity=" + game.geralt.terminalVelocity);
        writer.WriteLine(";how long your jumps will maintain jump velocity (seconds)");
        writer.WriteLine("jumpTimeMax=" + game.geralt.jumpTimeMax);
        writer.WriteLine("");

        writer.WriteLine(";highly unrecommended to not mess with these unless you really know what you're doing");
        writer.WriteLine(";collider size and placement relative to the 128x128 player sprite");
        writer.WriteLine("XcolliderOffset=" + game.geralt.XcolliderOffset);
        writer.WriteLine("YcolliderOffset=" + game.geralt.YcolliderOffset);
        writer.WriteLine("colliderWidth=" + game.geralt.colliderWidth);
        writer.WriteLine("colliderHeight=" + game.geralt.colliderHeight);
        writer.WriteLine("");

        writer.WriteLine(";this is the size of the rectangle used as the main attack hitbox");
        writer.WriteLine("attackColliderWidth=" + game.geralt.attackColliderWidth);
        writer.WriteLine("attackColliderHeight=" + game.geralt.attackColliderHeight);
        writer.WriteLine("");

        writer.WriteLine(";self-explanatory");
        writer.WriteLine("maxHealth=" + game.geralt.maxHealth);
        writer.WriteLine("maxShield=" + game.geralt.maxShield);
        writer.WriteLine("maxEnergy=" + game.geralt.maxEnergy);
        writer.WriteLine("");



        writer.WriteLine(";the distance at which the shockwave stops gaining power (otherwise it would be insanely strong close-up)");
        writer.WriteLine("shockwaveMaxEffectDistance=" + game.geralt.shockwaveMaxEffectDistance);
        writer.WriteLine(";distance at which shockwave has no power");
        writer.WriteLine("shockwaveEffectiveDistance=" + game.geralt.shockwaveEffectiveDistance);
        writer.WriteLine(";shockwave multipliers");
        writer.WriteLine("shockwaveStunTime=" + game.geralt.shockwaveStunTime);
        writer.WriteLine("shockwaveKnockback=" + game.geralt.shockwaveKnockback);
        writer.WriteLine("");

        writer.WriteLine(";this how the player won't take damage after previously taking damage (seconds)");
        writer.WriteLine("invulnerableMaxTime=" + game.geralt.invulnerableMaxTime);
        writer.WriteLine("");

        writer.WriteLine(";these are only the base. they may be modified in-game by potions and other ways (per second)");
        writer.WriteLine("shieldRechargeRate=" + game.geralt.shieldRechargeRate);
        writer.WriteLine("energyRechargeRate=" + game.geralt.energyRechargeRate);
        writer.WriteLine("healthRechargeRate=" + game.geralt.baseHealing);
        writer.WriteLine("potionRechargeRate=" + game.geralt.potionRechargeRate);
        writer.WriteLine("potionRechargeTime=" + game.geralt.potionRechargeTime);
        writer.WriteLine("maxNumberOfPotions=" + game.geralt.maxNumberOfPotions);
        writer.WriteLine("");

        writer.WriteLine(";this is the percentage of the damage absorbed by the shield that is converted back into health");
        writer.WriteLine(";0.25 means 25% of damage absorbed by the shield is converted to health (if 20 damage is absorbed, 5 damage is healed)");
        writer.WriteLine("shieldHealingPercentage=" + game.geralt.shieldHealingPercentage);
        writer.WriteLine("");

        writer.WriteLine(";how full the energy bar has to be before allowing you to use (95 == 95%)");
        writer.WriteLine("energyUsableMargin=" + game.geralt.energyUsableMargin);
        writer.WriteLine("");
        //writer.WriteLine("");
        ...
        for (int i = 00; i < 20; i++)
        {
            writer.WriteLine("animationSpeed[" + i + "]=" + game.geralt.animationSpeed[i]);
        }
        writer.WriteLine("");
        ...
        for (int i = 00; i < 20; i++)
        {
            writer.WriteLine("animationFrames[" + i + "]=" + game.geralt.animationFrames[i]);
        }
        writer.WriteLine("");
        ...
        writer.WriteLine(";NOTE: there is no variable for the number of frames in the shield animation, as the shield animator");
        writer.WriteLine(";uses the width of the shield sprite to determine when to loop.");
        writer.WriteLine("shieldAnimationSpeed=" + game.geralt.shieldAnimationSpeed);


        writer.WriteLine(";FONT SETTINGS");
        if (debug0) { Console.WriteLine(";FONT SETTINGS"); }
        writer.WriteLine(";the height of each character in the font spritesheet (pixels)");
        writer.WriteLine("characterHeight=" + game.font.charHeight);
        writer.WriteLine(";the width of each character in the font spritesheet (pixels)");
        for (int i = 00; i < 100; i++)
        {
            writer.WriteLine("characterWidth[" + i + "]=" + game.font.charWidth[i]);
        }
        writer.WriteLine(";characterWidth[99] is used as SPACE, so it should remain blank on the spritesheet (it is the final character)");
        writer.WriteLine("");
        writer.WriteLine("");
        ...
        writer.WriteLine(";DEBUG MODE");
        if (debug0) { Console.WriteLine(";DEBUG MODE"); }
        writer.WriteLine("debug=" + game.debug);
        writer.WriteLine("");
        writer.WriteLine(";//END");
    ...
    public static PlayerSettings Load(string filename)                                          //CHANGE THIS BEFORE SHIPPING 
    {
        ...
        {
            if (debug0) { Console.WriteLine("     line: " + line); }
            line.Trim();
            //line = line.ToLower();
            ...
            if (line.Length >= 1 && !line[0].Equals('\u003b'))
            {
                ...
                if (!string.IsNullOrWhiteSpace(variable) && !string.IsNullOrWhiteSpace(value))
                {
                    if (debug0)
                    {
                        if (string.IsNullOrWhiteSpace(extra))
                        {
                            ...
                            Console.WriteLine("  restate: " + variable + "=" + value);
                        }
                        else
                        {
                            Console.WriteLine("statement: " + statement);
                            ...
                            Console.WriteLine("    value: " + value);
                            Console.WriteLine("  restate: " + variable + "[" + extra + "]=" + value);
                        }
                    }
                    switch (variable.ToLower())
                    {
                        case "attack1damage":
                            ...
                            {
                                Console.WriteLine("error: variable \"" + variable + "\" could not be parsed");
                                errorVars = errorVars + "\n  name:" + variable + "\n    value:" + value;
                            }
                        ...
                        case "animationspeed":
                            if (float.TryParse(value, out floatResult))
                            {
                                ...
                                {
                                    Console.WriteLine("error: variable index \"" + variable + "[" + extra + "]\" could not be parsed");
                                    errorVars = errorVars + "\n  " + variable + "[" + extra + "]\n    value:" + value;
                                }
                            }
                            else
                            {
                                Console.WriteLine("error: variable \"" + variable + "[" + extra + "]\" could not be parsed");
                                errorVars = errorVars + "\n  " + variable + "[" + extra + "]\n    value:" + value;
                            }
                            break;
                        ...
                        default:
                            if (string.IsNullOrWhiteSpace(extra))
                            {
                                Console.WriteLine("error: no variable with name: \"" + variable + "\"");
                                errorVars = errorVars + "\n  name:" + variable + "\n    value:" + value;
                            }
                            else
                            {
                                Console.WriteLine("error: no variable with name: \"" + variable + "[" + extra + "]\"");
                                errorVars = errorVars + "\n    " + variable + "[" + extra + "]";
                            }
                            break;
                    }
                ...
                {
                    if (debug0) { Console.WriteLine("--ignored improper format"); }
                }
            }
            else
            {
                if (debug0) { Console.WriteLine("--ignored comment or empty line"); }
            }
        }
        ...
        if (debug0) { Console.WriteLine("load directory: " + ((FileStream)(reader.BaseStream)).Name); }
        reader.Close();
    ...
    private static Vector2 Vector2Parser(string value)
    {
        ...
        if (float.TryParse(Xval, out float Xresult))
        {
            ...
            {
                Console.WriteLine("error: Vector2 could not be parsed");
            }
        }
        else
        {
        ...
        if (int.TryParse(Xval, out int Xresult))
        {
            if (int.TryParse(Yval, out int Yresult))
            {
                if (int.TryParse(widthVal, out int widthResult))
                {
                    ...
                    {
                        Console.WriteLine("error: Rectangle could not be parsed");
                    }
                }
                else
                {
