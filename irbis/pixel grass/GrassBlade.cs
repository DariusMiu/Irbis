...
public class GrassBlade
{
    ...
    public void Update()
    {
        ...
        float distanceSqr = Irbis.Irbis.DistanceSquared(Irbis.Irbis.jamie.BottomCenter, parentGrass.position + position);
        if (distanceSqr <= parentGrass.brushDistanceSqr)
        {
            if (Irbis.Irbis.Directions(Irbis.Irbis.jamie.BottomCenter, parentGrass.position + position) == Direction.Right)
    ...
    { sb.Draw(parentGrass.bladeTextures, position, bladeTextureArea, Color.White, rotation + brushRotation, parentGrass.bladeOrigin, 1f, SpriteEffects.None, depth); }
